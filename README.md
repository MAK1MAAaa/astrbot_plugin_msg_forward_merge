...
- **定时合并转发**：可按规则开启消息缓冲，将指定秒数内的多条消息合并为一条后再转发，不会发送空消息。
...
开启后，插件从该规则收到第一条符合过滤条件的消息时开始计时；在接下来的 `10` 秒内收到的消息会按原顺序合并为一条消息链，在窗口结束后统一发送。规则未开启 `hide_header` 时，每条原消息仍保留各自的来源信息头。
