...
- **消息过滤**：支持对转发规则设置消息过滤模式（off/blacklist/whitelist），并配置过滤正则表达式或关键词。
- **定时合并转发**：可按规则开启消息缓冲，将指定秒数内的多条消息合并为一条后再转发；QQ OneBot 规则会自动发送原生合并转发记录，不会发送空消息。
...
| `default_cooldown_seconds` | int | `0` | 默认转发冷却时间，0 为关闭 |
...
| `cooldown_seconds` | int | 转发冷却时间；未设置时继承全局默认值 |
| `merge_forward_enabled` | bool | 是否开启定时合并转发，默认关闭 |
| `merge_forward_seconds` | int | 合并窗口秒数，默认 `10`；仅开关开启时生效，必须大于 `0` |
...
| `cooldown_seconds` | 转发冷却时间；未设置时继承全局默认值 |
| `merge_forward_enabled` | 是否开启定时合并转发 |
| `merge_forward_seconds` | 合并窗口秒数；仅开关开启时生效，必须大于 `0` |
...
开启后，插件从该规则收到第一条符合过滤条件的消息时开始计时；在接下来的 `10` 秒内收到的消息会按原顺序合并，在窗口结束后统一发送。

当源会话和目标会话的适配器类型均为 `aiocqhttp`（QQ OneBot，例如 NapCat）时，插件会把每条消息构造成一个 `Node`，发送 QQ 原生“合并转发”聊天记录。平台实例 ID 可以是 `default` 等自定义值，无需写成 `aiocqhttp`。节点会显示原发送者昵称和 QQ 号，不再附加重复的 `[转发]` 来源信息头。其他平台组合仍使用普通合并消息链，以保持跨平台兼容。

没有消息到达时不会创建发送任务，也不会向目标会话发送空消息。时间窗口结束后，下一条消息会开启新的窗口。

合并窗口内的图片和语音会在收到时立即固化，避免 AstrBot 清理临时媒体文件后导致发送失败。
