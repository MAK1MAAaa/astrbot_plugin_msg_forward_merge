...
| `mt listall` | 列出所有转发规则         |
| `mt hide`    | 切换规则的来源信息显示状态     |
| `mt hidelist` | 查看所有规则的来源信息显示状态 |
| `mt help`    | 显示该插件帮助信息        |
...
**使用方法**：
```
#mt hide <规则ID>          # 切换规则的来源信息显示状态
#mt hidelist                 # 查看所有规则的来源信息显示状态
```

`#mt list` 命令会显示每个规则的状态标记：
- 🔓 表示允许显示来源信息
- 🔒 表示禁止显示来源信息
...
```
由 mt add / mt bind 创建，或直接在 WebUI 配置页面添加。
```
...
所有规则保存在 AstrBot 的插件配置文件中，同时也同步到 `rules.json`。
